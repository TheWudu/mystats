# frozen_string_literal: true

module Repositories
  module Statistics
    module YearOverYear
      def yoy_execute(grouping)
        data = yoy_aggregation(grouping)

        years.each_with_object([]) do |year, result|
          result << { name: year, data: fill_days(year, grouping, data) }
        end
      end

      def yoy_aggregation(grouping)
        group = if grouping == 'week'
                  { year: '$iso_week_year', week: '$iso_week' }
                else
                  { year: '$iso_week_year', day: '$date' }
                end
        data = sessions.aggregate([
                                    { '$match' => matcher },
                                    { '$addFields' => {
                                      timezone: { '$ifNull' => ['$timezone', 'UTC'] },
                                       date: { '$dateToString' => { date:     '$start_time',
                                                                    timezone: '$timezone',
                                                                    format:   '%Y-%m-%d' } },
                                      'iso_week'      => { '$isoWeek' => '$start_time' },
                                      'iso_week_year' => { "$isoWeekYear": '$start_time' }
                                    } },
                                    { '$group' => { _id:      group,
                                                    distance: { '$sum' => '$distance' } } },
                                    { '$sort' => { _id: 1 } }
                                  ]).to_a
      end

      def group_key(grouping, doc)
        case grouping
        when 'day'
          Date.parse(doc['_id']['day']).strftime('%-d.%-m.')
        when 'week'
          doc['_id']['week']
        end
      end

      def assign_data(result, year, grouping, data)
        data.each do |d|
          next if d.dig('_id', 'year') != year

          key = group_key(grouping, d)
          result[key] = d['distance'] / 1000.0
        end
      end

      def fill_days(year, grouping, data)
        r = case grouping
            when 'day'
              initialize_days(year)
            when 'week'
              initialize_weeks
            else
              raise NotImplementedError
            end

        assign_data(r, year, grouping, data)
        apply_sums(r)
      end

      def initialize_days(year)
        (Date.parse("#{year}-01-01")..Date.parse("#{year}-12-31")).each_with_object({}) do |day, hash|
          hash["#{day.day}.#{day.month}."] = 0
        end
      end

      def initialize_weeks
        (0..52).each_with_object({}) do |week, hash|
          hash[week] = 0
        end
      end

      def apply_sums(result)
        year_sum = 0

        result.each do |k, v|
          year_sum += v
          result[k] = year_sum.round(2)
        end
        result
      end
    end
  end
end
